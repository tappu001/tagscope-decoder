  ...
  function tagNames(t, ctx) {
    ...
    switch (t.fn) {
      ...
      case '__html': {
        ...
        const known = hi.events.join(', ') || (hi.baseOnly ? 'Base Code' : '') || hi.comment || (hi.dlEvents.length ? `push ${hi.dlEvents.join(', ')}` : '') || (hi.loads ? `loads ${hi.loads}` : '');
        let detail = known || (main ? context : hi.snippet || context);
        if (main && main.name === 'Google tag (gtag.js)' && hi.sendTo) {
          detail = `${hi.events.join(', ') || 'event'} ${hi.sendTo}`;
          const plat = /^AW-/.test(hi.sendTo) ? ['Google Ads (gtag)', 'GAds'] : ['GA4 (gtag)', 'GA4'];
          return both(`${plat[0]} - ${detail}`, `${plat[1]} - cHTML - ${detail}`);
        }
        if (main) return both(`${main.name} - ${detail}`, `${main.short} - cHTML - ${detail}`);
        return both(`Custom HTML - ${detail}`, `cHTML - ${detail}`);
      }
      case '__img': {
